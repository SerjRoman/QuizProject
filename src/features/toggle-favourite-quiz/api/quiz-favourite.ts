...
import type {
	QuizFavouritePayload,
	QuizToggleFavouritePayload,
} from "./api.types";

const quizFavouriteApi = baseApi.injectEndpoints({
	endpoints: (build) => {
		return {
			toggleFavourite: build.mutation<void, QuizToggleFavouritePayload>({
				query: (body) => ({
					url: QUIZ_FAVOURITE_API_MAP.toggleFavourite,
					method: "PATCH",
					body,
				}),
				...
				invalidatesTags: (_, __, arg) => [
					{ type: "LibraryQuiz", id: arg.quizId },
				],
			}),
